...
public class AnnouncementDAOImpl implements AnnouncementDAO {
    ...
    public List getAllMyNotification(long id){
        return sessionFactory.getCurrentSession().createSQLQuery("Select Announcement.description, Announcement.subject, Announcement.announcement_id, Announcement.posted_date, Announcement.user_id, Announcement.user_assigned\n" +
                "FROM Announcement\n" +
                "INNER JOIN User_Notifications ON Announcement.announcement_id = User_Notifications.anunt_id\n" +
                "INNER JOIN User ON Announcement.user_id = User.user_id\n" +
                "WHERE User_Notifications.user_id = :id").addEntity(Announcement.class).setParameter("id",id).list();
    }

    @Override
    public List getAnnouncementsByCurrentDate(String date) {
        return sessionFactory.getCurrentSession().createSQLQuery("Select Announcement.description, Announcement.subject, Announcement.announcement_id, Announcement.posted_date, Announcement.user_id, Announcement.user_assigned from Announcement\n" +
                " where date(posted_date) = :data").addEntity(Announcement.class).setParameter("data",date).list();
    }
